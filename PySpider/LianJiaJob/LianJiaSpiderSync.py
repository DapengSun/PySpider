...
def saveJobResult():
    try:
        ...
        _description = [i[0] for i in cursor.description]
        ...
        for item in _resultDict:
            ...
            if len(_resultList) > 0:
                for item in _resultList:
                    ...
                    _searchInfoModel = SearchInfoModel(_itemList['SearchId'], _itemList['HouseInfoUrl'],
                                                       _itemList['HouseInfoCode'], _itemList['SurfacePlotThumbnail'],
                                                       _itemList['HouseTitle'], _itemList['HouseTag'],
                                                       _itemList['HouseAddressUrl'],
                                                       _itemList['HouseAddressName'], _itemList['HousePattern'],
                                                       _itemList['HouseSize'], _itemList['HouseOrientation'],
                                                       _itemList['HouseCover'], _itemList['HouseElevator'],
                                                       _itemList['HouseFloor'],
                                                       _itemList['HouseYear'], _itemList['HouseArea'],
                                                       _itemList['HousePeoperNum'], _itemList['HouseLookNum'],
                                                       _itemList['HouseFollowSubway'], _itemList['HouseTaxFree'],
                                                       _itemList['HouseHasKey'],
                                                       _itemList['HouseTotalPrice'], _itemList['HouseUnitPrice'])

                    saveHouseInfo(_searchInfoModel,db,cursor)

                _updateSearchSql = "Update searchinfo Set Status = '%s' Where SearchId = '%s'" % (
                SpiderJobStatus.结果入库.value, _currentSearchId)
                cursor.execute(_updateSearchSql)
                db.commit()
        ...
        print(ex)
        _updateSearchSql = "Update searchinfo Set Status = '%s' Where SearchId = '%s'" % (
        SpiderJobStatus.异常.value, _currentSearchId)
        cursor.execute(_updateSearchSql)
        db.commit()
    finally:
        cursor.close()
...
def saveHouseInfo(SearchInfoModel,db,cursor):
    _insertSql = "insert into `searchdetailinfo`(SearchId,HouseInfoUrl,HouseInfoCode,SurfacePlotThumbnail,HouseTitle,HouseTag,HouseAddressUrl,HouseAddressName," \
                 ...
                 (SearchInfoModel.SearchId, SearchInfoModel.HouseInfoUrl, SearchInfoModel.HouseInfoCode,
                  SearchInfoModel.SurfacePlotThumbnail, SearchInfoModel.HouseTitle,
                  SearchInfoModel.HouseTag, SearchInfoModel.HouseAddressUrl, SearchInfoModel.HouseAddressName,
                  SearchInfoModel.HousePattern, SearchInfoModel.HouseSize,
                  SearchInfoModel.HouseOrientation, SearchInfoModel.HouseCover, SearchInfoModel.HouseElevator,
                  SearchInfoModel.HouseFloor, SearchInfoModel.HouseYear,
                  SearchInfoModel.HouseArea, SearchInfoModel.HousePeoperNum, SearchInfoModel.HouseLookNum,
                  SearchInfoModel.HouseFollowSubway,
                  SearchInfoModel.HouseTaxFree, SearchInfoModel.HouseHasKey, SearchInfoModel.HouseTotalPrice,
                  SearchInfoModel.HouseUnitPrice,
